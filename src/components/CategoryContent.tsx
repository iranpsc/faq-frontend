...
export default function CategoryContent({ 
  ...
  const [category, setCategory] = useState<CategoryWithChildren | null>(initialCategory);
  ...
  useEffect(() => {
    ...
    if (initialPage !== pagination.meta.current_page) {
      fetchData(slug, initialPage);
    }
  }, [slug, searchParams, pagination.meta.current_page]);
  ...
  if (!category) {
    return (
      <ContentArea layout="with-sidebar" showSidebar={true} mainWidth="3/4" sidebarWidth="1/4" sidebar={<HomeSidebar />}>
        <BaseAlert variant="error" message="دسته‌بندی یافت نشد" />
      </ContentArea>
    );
  }
